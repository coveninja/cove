...
for (const [index, page] of (manifest.pages ?? []).entries()) {
	...
	if (!Number.isInteger(page.order) || page.order < 0) fail(`${label}.order must be a non-negative integer`);
	if (page.keywords !== undefined && !Array.isArray(page.keywords)) {
		fail(`${label}.keywords must be an array when present`);
	}
