...
func Interfaces() ([]net.Interface, error) {
	...
	// Note: zoneCache update removed — //go:linkname to net.zoneCache is
	// rejected by Go 1.23+ (the target symbol no longer opts in to linkname).
	// IPv6 zone-ID lookups may fall back to numeric form on Android 11+; this
	// is an acceptable trade-off for a torrent/HTTP streaming backend that
	// does not rely on scoped IPv6 multicast.
	return ift, nil
}
...
func InterfaceByName(name string) (*net.Interface, error) {
	...
	// Note: zoneCache update removed — see Interfaces() comment.
	for _, ifi := range ift {
		if name == ifi.Name {
			return &ifi, nil
		}
